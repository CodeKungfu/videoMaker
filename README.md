...
- **全局 AI 引擎配置**：支持在工作台直接配置本地 Stable Diffusion WebUI、ComfyUI 服务地址，或接入 OpenAI、Midjourney 的云端 API Token，配置数据仅存本地 SQLite，安全可控。
...
- 🏭 **镜头生产车间**：
  ...
  - **支持手动编辑 Prompt**，高度可控。
  - **抽卡工作流 (Gacha Grid)**：支持一键生成多张候选图（2x2 网格），从中挑选最满意的一张（Pick）为定稿；不满意随时修改提示词重新抽卡（Re-roll）。
